...
export default function GallerySection({data}: {data: GallerySectionType}) {
  return (
    <section className="py-16">
      <Container>
        {data.title && <h2 className="text-center mb-10">{data.title}</h2>}
        <div className={`grid gap-4 ${data.layout === 'carousel' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4' : 'grid-cols-2 md:grid-cols-3'}`}>
          {data.images?.map((img, i) => (
            <div key={i} className="relative aspect-square rounded-lg overflow-hidden group">
              <SanityImage image={{_type: 'image', asset: {...img.asset, _type: 'reference'}, alt: img.alt}} fill sizes="(max-width:768px) 50vw, 33vw" className="object-cover transition-transform duration-300 group-hover:scale-105" />
              {img.caption && (
